...
- Редактирование ассортимента в WebX Panel (вкладка Shop)
...
### ⚡ Quests (0.1.0)
Командная система заданий, которую настраивает администрация

**Функции:**
- Квесты делаются группами игроков или одиночно
- Администратор добавляет/редактирует задания через режим редактирования WebX Panel
- Гибкие цели: убийства, крафт, сбор, метрики из других плагинов
- Награды настраиваются (монеты, опыт, предметы, очки клана)
- Прогресс отображается в игре и на Dashboard

**Команды:**
- `/quests list` — список доступных заданий
- `/quests accept <quest>` — принять выбранный квест
- `/quests info <quest>` — подробности и прогресс
- `/quests abandon <quest>` — отказаться

> Плагин не содержит предустановленных миссий — все сценарии создаёт Admin Server через WebX Panel.
...
### 🎓 Skills (0.1.0)
Полностью настраиваемая система навыков с деревьями прогресса

**Функции:**
- Навыки открываются в процессе игры (бой, профессии, достижения)
- Каждая ветка даёт игровые бонусы: урон, скорость, крафт, защита и пр.
- Базовые деревья поставляются из коробки и могут быть переписаны в `config.yml`
- Поддержка требований (уровень, квесты, экономия очков) и ограничений на количество активных навыков
- Совместимо с WebX Panel: можно просматривать статистику разблокировок

**Конфигурация:**
- Настраивайте категории, требования и эффекты через YAML/JSON конфиг
- Поддерживаются локализации и собственные иконки для GUI меню

[📖 Подробнее](packages/skills/README.md)

---

### 🛏️ BedWars Event (0.2.0)
Расширенный движок BedWars-ивентов с очисткой арен

**Функции:**
- Автоматическая очистка и восстановление карт после каждого матча
- «Фокусы» — сценарии с особыми условиями (например, sudden focus на конкретную команду)
- Таймеры событий, кастомные лут-пулы и динамическое изменение правил
- API для подключения собственных мини-режимов и наград
- Совместимость с основным BedWars набором арен сервера

[📖 Подробнее](packages/bedwarsevent/README.md)
