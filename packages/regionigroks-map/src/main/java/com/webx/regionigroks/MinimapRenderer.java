...
public class MinimapRenderer extends MapRenderer {
    ...
    private byte getBlockColor(Material mat) {
        ...
        if (mat == Material.GRASS_BLOCK || mat == Material.SHORT_GRASS || mat == Material.TALL_GRASS) {
            ...
            return MapPalette.matchColor(new Color(110, 72, 38)); // Dark soil
            ...
            return MapPalette.matchColor(new Color(230, 214, 160)); // Sandstone beige
        } else if (mat == Material.CLAY) {
            return MapPalette.matchColor(new Color(180, 200, 209)); // Clay gray-blue
        } else if (mat.name().contains("TERRACOTTA")) {
            return MapPalette.matchColor(new Color(191, 141, 110)); // Terracotta warm
        } else if (mat.name().endsWith("_WOOL")) {
            return MapPalette.matchColor(new Color(220, 220, 220)); // Neutral wool
        } else if (mat.name().endsWith("_CONCRETE")) {
            return MapPalette.matchColor(new Color(180, 180, 180)); // Neutral concrete
        } else if (mat == Material.WATER || mat == Material.SEAGRASS || mat == Material.KELP || mat == Material.KELP_PLANT) {
            return MapPalette.matchColor(new Color(0, 105, 148)); // Water blue
            ...
            return MapPalette.matchColor(new Color(193, 154, 107)); // Light wood
            ...
            return MapPalette.matchColor(new Color(210, 180, 100)); // Path tan
        } else if (mat == Material.NETHERRACK) {
            return MapPalette.matchColor(new Color(153, 51, 51)); // Nether red
        } else if (mat == Material.SOUL_SAND || mat == Material.SOUL_SOIL) {
            return MapPalette.matchColor(new Color(115, 78, 48)); // Soul brown
        } else if (mat == Material.END_STONE) {
            return MapPalette.matchColor(new Color(236, 235, 185)); // Pale yellow
        } else if (mat == Material.BLACKSTONE || mat == Material.BASALT) {
            return MapPalette.matchColor(new Color(54, 57, 63)); // Dark basalt
